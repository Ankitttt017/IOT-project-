...
const OperationsTab = ({ part, partId, onSheetsChange }) => {
  const { t } = useI18n();
              ...
              {part.operations.length > 0 ? (
                part.operations.map((op) => (
                  <tr key={op.sr} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-600 text-xs font-mono">{op.label}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button className="text-teal-600 hover:text-teal-700 transition-colors" title={t("edit")}>
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                          </svg>
                        </button>
                        <button className="text-red-400 hover:text-red-600 transition-colors" title={t("delete")}>
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
